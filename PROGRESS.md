...
- [IN PROGRESS] UI final de todas as telas — 14 de 19 experiências do GDD implementadas, todas construídas em runtime (`GameShellBuilder` + `ScreenManager` + `UiFactory`) e ligadas na API real de `GameRoot`: Office (hub), Products, Employees (contratar), Upgrades (comprar), Store (gastar gems), Finances (aceitar investimento), Statistics (`StatisticsSnapshot` completo), Achievements, Missions, Settings (volume de áudio por categoria), Research (11 trilhas estudáveis), Company (estágio/equity/rodadas/concorrentes), Character (perfil/ranking) e Events (modal de escolha sobre qualquer tela). Faltam: Splash, Main Menu, New Game, Continue e uma tela dedicada de Development; o loop de desenvolvimento já funciona no Office.
