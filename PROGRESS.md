...
- [IN PROGRESS] UI final de todas as telas — 10 de 19 telas do GDD implementadas e navegáveis, todas construídas em runtime (`GameShellBuilder` + `ScreenManager` + `UiFactory`) e ligadas na API real de `GameRoot`: Office (hub), Products, Employees (contratar), Upgrades (comprar), Store (gastar gems), Finances (aceitar investimento), Statistics (`StatisticsSnapshot` completo), Achievements, Missions, Settings (volume de áudio por categoria). Faltam: Splash, Main Menu, New Game, Continue, Character, Research (árvore visual), Company, Events (modal de escolha).
- [PENDING] Art polish — visual atual é placeholder funcional (retângulos/texto padrão), sem direção de arte própria ainda
- [COMPLETED] Android build (APK/AAB) — **APK real gerado e confirmado**: `Builds/Android/StartupEmpire-debug.apk`, 32.189.060 bytes, `result=Succeeded totalErrors=0 totalWarnings=0`, ~5m44s de build (IL2CPP + Gradle reais). Confirmado com o comando `file` (reconhecido como "Android package (APK)"), não é alegação sem evidência. AAB (formato de publicação na Play Store) ainda não gerado — só o APK de desenvolvimento.
...
- [COMPLETED] Unity Test Framework (EditMode + PlayMode) — **24/24 EditMode + 4/4 PlayMode reais passando** (115 testes reais no total com os 87 do `Tests.NET`), rodados via `Unity.exe -batchmode -runTests`. EditMode cobre uma amostra representativa (Economy, Development, Customer Acquisition, Save, Idle, Progression, Missions/Achievements) portada do `Tests.NET`. PlayMode testa a UI de verdade em Play Mode: cria a tela inteira, clica em "Estudar" (confirma que o conhecimento aumenta), navega para Products pelo menu (confirma que a tela certa fica visível e a outra escondida), contrata um funcionário pela tela Employees (confirma que o roster cresce). Pegou 3 bugs reais no processo (ver seção de bugs abaixo). Exigiu separar o código do jogo em seu próprio assembly definition (`StartupEmpire.Game.asmdef`) e adicionar `[InternalsVisibleTo]` para os testes acessarem os setters `internal`.
