...
- [IN PROGRESS] UI final de todas as telas — 1 de 19 telas do GDD implementada e jogável (Office/hub: caixa, valuation, MRR, gems, estágio, status do produto, botões Estudar/Desenvolver/Corrigir Bugs/Lançar/Avançar Ciclo, tudo construído em runtime e testado em PlayMode de verdade). As outras 18 (Splash, Menu, Character, Employees, Research, Missions, Upgrades, Finances, Statistics, Company, Events, Achievements, Store, Settings, etc.) ainda faltam.
...
- [COMPLETED] Unity Test Framework (EditMode + PlayMode) — **24/24 EditMode + 2/2 PlayMode reais passando**, rodados via `Unity.exe -batchmode -runTests`. EditMode cobre uma amostra representativa (Economy, Development, Customer Acquisition, Save, Idle, Progression, Missions/Achievements) portada do `Tests.NET`; o resto da cobertura (87/87) continua no `Tests.NET`. PlayMode testa a tela Office de verdade em Play Mode (constrói a UI, clica no botão "Estudar", confirma que o conhecimento aumentou) — pegou um bug real na primeira tentativa (ver abaixo). Exigiu separar o código do jogo em seu próprio assembly definition (`StartupEmpire.Game.asmdef`) e adicionar `[InternalsVisibleTo]` para os testes acessarem os setters `internal`.
